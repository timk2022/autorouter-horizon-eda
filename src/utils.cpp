...
uint32_t num_components_in_top_block(json j) {
  ...
  for (auto comp = j["components"].begin(); comp != j["components"].end();
       comp++) {
    count++;
  }
  return count;
...
void print_component_group(struct component_group_t *component_arr) {
  std::cout
      << "=================COMPONENT GROUP DEBUG OUTPUT===================\n";
  for (int i = 0; i < component_arr->comp_arr.size(); i++) {
    std::cout << "================= component number: " << i
              << " ===================\n";
    std::cout << "component id: "
              << std::string(component_arr->comp_arr[i].component_id)
              << std::endl;
    component_arr->comp_arr[i].pos_offset.print();
    std::cout << "component angle: " << component_arr->comp_arr[i].angle
              << std::endl;
    std::cout << std::endl;
    std::cout << "part id: " << std::string(component_arr->comp_arr[i].part_id)
              << std::endl;
    std::cout << "entity id: "
              << std::string(component_arr->comp_arr[i].entity_id) << std::endl;
    std::cout << "group id: " << std::string(component_arr->comp_arr[i].group)
              << std::endl;
    std::cout << "tag id: " << std::string(component_arr->comp_arr[i].tag)
              << std::endl;
    ...
    for (int j = 0; j < component_arr->comp_arr[i].conn_arr.size(); j++) {
      std::cout << "----------------- connection number: " << j
                << " ----------------------\n";
      std::cout << "conn gate: "
                << std::string(component_arr->comp_arr[i].conn_arr[j].gate)
                << std::endl;
      std::cout << "conn pin: "
                << std::string(component_arr->comp_arr[i].conn_arr[j].pin)
                << std::endl;
      std::cout << "conn net: "
                << std::string(component_arr->comp_arr[i].conn_arr[j].net)
                << std::endl;
    }
  }
}
...
void print_net_list(struct net_group_t *net_list) {
  ...
  for (uint32_t i = 0; i < net_list->nets.size(); i++) {
    std::cout << "================= net number: " << i
              << " ===================\n";
    std::cout << "net id: " << std::string(net_list->nets[i].net_id)
              << std::endl;
    for (uint32_t j = 0; j < net_list->nets[i].linked_conns_arr.size(); j++) {
      std::cout << "----------------- linked component number: " << j
                << "-------------------\n";
      std::cout << "component id: "
                << std::string(net_list->nets[i].linked_conns_arr[j].comp_id)
                << std::endl;
      std::cout << "net id: "
                << std::string(net_list->nets[i].linked_conns_arr[j].net)
                << std::endl;
      std::cout << "gate id: "
                << std::string(net_list->nets[i].linked_conns_arr[j].gate)
                << std::endl;
      std::cout << "pin id: "
                << std::string(net_list->nets[i].linked_conns_arr[j].pin)
                << std::endl;
      std::cout << "package id: "
                << std::string(net_list->nets[i].linked_conns_arr[j].package_id)
                << std::endl;
      std::cout << "pad id: "
                << std::string(net_list->nets[i].linked_conns_arr[j].pad_id)
                << std::endl;
      std::cout
          << "part id: "
          << std::string(
                 net_list->nets[i].linked_conns_arr[j].comp_pointer->part_id)
          << std::endl;

      std::cout << "pad angle: "
                << net_list->nets[i].linked_conns_arr[j].pad_angle << std::endl;
...
component_group_t *load_top_block(const std::string &filename) {
  ...
  for (auto comp = loaded_json["components"].begin();
       comp != loaded_json["components"].end(); comp++) {
    std::string component_id = comp.key();
    uint32_t num_connections =
        num_conns_in_comp_top_block(loaded_json, component_id);
    ...
    // populate components
    for (auto conn =
             loaded_json["components"][component_id]["connections"].begin();
         conn != loaded_json["components"][component_id]["connections"].end();
         conn++) {
  ...
  for (auto i = 0; i < components->comp_arr.size(); i++) {
    for (auto j = 0; j < components->comp_arr[i].conn_arr.size(); j++) {
      components->comp_arr[i].conn_arr[j].comp_pointer =
          &(components->comp_arr[i]);
    }
  }
  return components;
}
...
net_group_t *net_generation(component_group_t *components) {
  ...
  for (auto comp = json_data["nets"].begin(); comp != json_data["nets"].end();
       comp++) {
    num_nets++;
  }
  ...
  for (auto i = 0; i < net_list->nets.size(); i++) {
    ...
    for (auto j = 0; j < components->comp_arr.size(); j++) {
      for (auto k = 0; k < components->comp_arr[j].conn_arr.size(); k++) {
        if (components->comp_arr[j].conn_arr[k].net ==
            net_list->nets[i].net_id) {
  ...
  for (auto i = 0; i < net_list->nets.size(); i++) {
    net_list->nets[i].linked_conns_arr.reserve(
        net_list->nets[i].linked_conns_arr_len);
    ...
    for (uint32_t j = 0; j < components->comp_arr.size(); j++) {
      for (uint32_t k = 0; k < components->comp_arr[j].conn_arr.size(); k++) {
        if (components->comp_arr[j].conn_arr[k].net ==
            net_list->nets[i].net_id) {
          // net_list->nets[i].linked_conns_arr[linked_index++] =
          // components->comp_arr[j].conn_arr[k];
          net_list->nets[i].linked_conns_arr.push_back(
              components->comp_arr[j].conn_arr[k]);
          // net_list->nets[i].linked_conns_arr[linked_index++].comp_id =
          // components->comp_arr[j].conn_arr[k].comp_id;
        }
      }
    }
  }
  ...
  for (auto i = 0; i < net_list->nets.size(); i++) {
    for (auto j = 0; j < net_list->nets[i].linked_conns_arr.size(); j++) {
      ...
      if (part_file.contains("base") && !part_file.contains("package")) {
        ...
        std::string base_part_filename =
            pool_base_path + "/parts/cache/" + base_part_id + ".json";
        // delete &part_file;
      ...
      // pad mapping
      for (auto pad_map = part_file["pad_map"].begin();
           pad_map != part_file["pad_map"].end(); pad_map++) {
        if (net_list->nets[i].linked_conns_arr[j].gate ==
                str_to_uuid(pad_map.value()["gate"]) &&
            net_list->nets[i].linked_conns_arr[j].pin ==
                str_to_uuid(pad_map.value()["pin"])) {
          net_list->nets[i].linked_conns_arr[j].pad_id =
              str_to_uuid(pad_map.key());
          break;
        }
      }
      ...
      if (package_file.contains("polygons")) {
        for (auto polygon_map = package_file["polygons"].begin();
             polygon_map != package_file["polygons"].end(); polygon_map++) {
          if (polygon_map.value()["parameter_class"] == "courtyard") {
            auto comp_pointer =
                net_list->nets[i].linked_conns_arr[j].comp_pointer;
            comp_pointer->courtyard.polygon_id = str_to_uuid(polygon_map.key());

            for (auto vert = polygon_map.value()["vertices"].begin();
                 vert != polygon_map.value()["vertices"].end(); vert++) {
              if (vert.value()["type"] == "line") {
                comp_pointer->courtyard.line_type.push_back(
                    polygon::POLYGON_STRAIGHT);
              } else if (vert.value()["type"] == "curve") {
                comp_pointer->courtyard.line_type.push_back(
                    polygon::POLYGON_CURVE);
              }
              comp_pointer->courtyard.vertices.push_back(
                  std::make_pair(Vec3_int(vert.value()["position"][0],
                                          vert.value()["position"][1], 0),
                                 Vec3_int(vert.value()["arc_center"][0],
                                          vert.value()["arc_center"][1], 0)));
              // todo: add curve support
            }
          }
        }
      } else {
        std::cout << "no polygons found in file for part "
                  << net_list->nets[i].linked_conns_arr[j].comp_pointer->part_id
                  << std::endl;
      }

      for (auto pad = package_file["pads"].begin();
           pad != package_file["pads"].end(); pad++) {
        if (net_list->nets[i].linked_conns_arr[j].pad_id ==
            str_to_uuid(pad.key())) {
          net_list->nets[i].linked_conns_arr[j].pad_angle =
              (double)pad.value()["placement"]["angle"];
          net_list->nets[i].linked_conns_arr[j].pad_offset.x =
              (double)pad.value()["placement"]["shift"][0];
          net_list->nets[i].linked_conns_arr[j].pad_offset.y =
              (double)pad.value()["placement"]["shift"][1];
          ...
          new_pad.vertices.push_back(std::make_pair(
              Vec3_int(-width / 2, -height / 2, 0), Vec3_int(0, 0, 0)));
...
void board_load_and_parse(component_group_t *comp_group,
  ...
  for (auto comp = board_file["packages"].begin();
       comp != board_file["packages"].end(); comp++) {
    for (uint32_t i = 0; i < comp_group->comp_arr.size(); i++) {
      // std::cout << comp.key() << std::endl;
      comp_group->comp_arr[i].is_used = false;
      if (comp_group->comp_arr[i].component_id ==
          str_to_uuid(comp.value()["component"])) {
        comp_group->comp_arr[i].board_comp_id = str_to_uuid(comp.key());
        comp_group->comp_arr[i].pos_offset.x =
            comp.value()["placement"]["shift"][0];
        comp_group->comp_arr[i].pos_offset.y =
            comp.value()["placement"]["shift"][1];
